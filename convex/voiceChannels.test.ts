...
test("a member without CONNECT (denied via channel override) is rejected by assertCanJoin", async () => {
  ...
  await expect(
    member.as.query(internal.voiceChannels.assertCanJoin, { channelId }),
  ).rejects.toThrow();

  // The owner is unaffected by the override.
  await expect(
    owner.as.query(internal.voiceChannels.assertCanJoin, { channelId }),
  ).resolves.toMatchObject({ activeRtkMeetingId: null });
});
